...
public class Wormhole extends PotionEffectWrapper {
    private final String defaultIcon;

    public Wormhole(String effect, String defaultIcon) {
    ...
    public void onInflict(LivingEntity p, LivingEntity causedBy, double amplifier, int duration, double intensity) {
        if (p instanceof Player pl) {
            Questionnaire questionnaire = new Questionnaire(pl, null, null,
                    new Question("&fQual o player que deseja se teleportar? (type in chat, or 'cancel' to cancel)", s -> !LootTableRegistry.getLootTables().containsKey(s), "&cPlayer não encontrado!")
            ) {
                @Override
                public Action<Player> getOnFinish() {
                    ...
                    return (p) -> {
                        pl.teleport(Objects.requireNonNull(Bukkit.getServer().getOnlinePlayers().stream().filter(cPlayer -> cPlayer.getDisplayName().toString().equals(question.getAnswer())).findFirst().orElse(null)).getLocation(), PlayerTeleportEvent.TeleportCause.PLUGIN);
                        pl.getWorld().playSound(pl.getLocation(), Sound.ENTITY_ENDERMAN_TELEPORT, 1F, 1F);
                    };
                }
            };
            Questionnaire.startQuestionnaire(pl, questionnaire);
        }
    }
    ...
    public PotionEffectWrapper copy() {
        return new Wormhole(getEffect(), defaultIcon).setCharges(charges);
    }
